...
router.post('/mood', ensureAuthenticated, async (req, res) => {
    try {
        ...
        const [entry, created] = await db.Entry.findOrCreate({
            where: {
                userId: req.user.id,
                date: new Date(date)
            },
            defaults: {
                userId: req.user.id,
                content: `Mood: ${mood}`, // Store mood in content for these specific entries
                mood: mood,
                date: new Date(date),
        ...
        // If the entry was found (not created) and its mood is different, update it
        if (!created && entry.mood !== mood) {
            entry.mood = mood;
            entry.content = `Mood: ${mood}`; // Also update content
            await entry.save();
        } 
        // If 'created' is true, findOrCreate already saved it with the correct mood from defaults.
