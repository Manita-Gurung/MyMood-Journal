...
router.put('/update/:id', ensureAuthenticated, async (req, res) => {
    try {
        ...
        if (!journal) {
            return res.status(404).json({ success: false, message: 'Journal entry not found or you do not have permission to edit it.' });
        }

        // Update the journal entry
        journal.title = title;
        journal.content = content;
        journal.date = date; // Ensure date is correctly formatted if needed by DB
