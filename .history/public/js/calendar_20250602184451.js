        ...
        moodButton.onclick = async () => {
            ...
            try {
                ...
                const response = await fetch(form.action, {
                    method: 'POST',
                    headers: {
                        'Accept': 'application/json', // Ensure server knows it's an AJAX request
                        // If you were using CSRF tokens, you'd add the token header here
                ...
                if (!response.ok) {
                    const errorResult = await response.json().catch(() => ({ message: 'Failed to submit mood. Server error.' }));
                ...
                if (result.success) {
                    ...
                    alert('Error updating mood: ' + (result.message || 'Unknown error'));
                }
                ...
                // Always remove dialog and overlay
