            ...
            .then(data => {
                ...
                if (data.recentEntries && data.recentEntries.length > 0) {
                    ...
                    data.recentEntries.forEach(entry => {
                        ...
                        const displayTitle = entry.title || (entry.content ? entry.content.substring(0, 30) + '...' : 'Journal Entry');
                        entriesHTML += `
                ...
                // Update total entries count in Quick Stats section
                const statItems = document.querySelectorAll('.stat-item');
                statItems.forEach(item => {
                    const label = item.querySelector('.stat-label');
                    if (label && label.textContent.trim() === 'Total Entries') {
                        const valueElement = item.querySelector('.stat-value');
                        if (valueElement && data.totalEntries !== undefined) {
                            valueElement.textContent = data.totalEntries;
                        }
                    }
                });
            })
            ...
            .then(data => {
                ...
                if (data.recentActivity && data.recentActivity.length > 0) {
                    ...
                    data.recentActivity.forEach(activity => {
                        // Use title if available, otherwise use generic title
                        const displayTitle = activity.title || 'Journal Entry';
                        activityHTML += `
